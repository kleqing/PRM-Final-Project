...
public class LoginActivity extends AppCompatActivity {
    ...
    private void signInWithEmail() {
        ...
        if ("admin@test.com".equals(email) && "123456".equals(password)) {
            Log.d(TAG, "Đăng nhập tài khoản admin test.");
            User adminUser = new User();
            adminUser.setRole("admin");
            adminUser.setEmail("admin@test.com");
            adminUser.setIsActive(true); // Đảm bảo tài khoản hoạt động
            redirectByRole(adminUser);
            return; // Dừng tại đây để không chạy code Firebase
        }
        // ---------------------------------
